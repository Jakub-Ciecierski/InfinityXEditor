...
void ImGui::PlotEx_xy(ImGuiPlotType plot_type, const char* label, float
(*values_getter)(void* data, int idx), void* data,void* data2, int
values_count, int
        values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
void ImGui::PlotLinesXY(const char* label, const float* values,const float*values2 ,int
    values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size, int stride)
{
    ImGuiPlotArrayGetterData data(values, stride);
    ImGuiPlotArrayGetterData data2(values2, stride);
    PlotEx_xy(ImGuiPlotType_Lines, label, &Plot_ArrayGetter,
